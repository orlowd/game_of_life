...
getScreenDimensionsFromOption(std::string_view window_size) {
    const auto split_pos = window_size.find('x');
    const auto width_str = std::string(window_size.substr(0, split_pos));
    const auto height_str = std::string(window_size.substr(split_pos + 1));
...
RunOptions parseOptions(int argc, char** argv) {
    ...
    try {
        ...
        std::tie(result.screen_width, result.screen_height)
            = getScreenDimensionsFromOption(opts_result["window"].as<std::string>());
